...
const (
	...
	//  session, _ := contacts.SessionStore.New(r, defaultSessionID)
...
func loginHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	oauthFlowSession, err := contacts.SessionStore.New(r, sessionID)
	if err != nil {
	...
	url := contacts.OAuthConfig.AuthCodeURL(sessionID, oauth2.ApprovalForce,
...
func oauthCallbackHandler(w http.ResponseWriter, r *http.Request) *appError {
	oauthFlowSession, err := contacts.SessionStore.Get(r, r.FormValue("state"))
	if err != nil {
	...
	tok, err := contacts.OAuthConfig.Exchange(context.Background(), code)
	if err != nil {
		return appErrorf(err, "could not get auth token: %v", err)
	}

	session, err := contacts.SessionStore.New(r, defaultSessionID)
	if err != nil {
...
func fetchProfile(ctx context.Context, tok *oauth2.Token) (*plus.Person, error) {
	client := oauth2.NewClient(ctx, contacts.OAuthConfig.TokenSource(ctx, tok))
...
func profileFromSession(r *http.Request) *Profile {
	session, err := contacts.SessionStore.Get(r, defaultSessionID)
	if err != nil {
		return nil
	}
