...
if [ -z "$BOOKSHELF_DEPLOY_LOCATION" ]; then
  echo "Must set \$BOOKSHELF_DEPLOY_LOCATION. For example: BOOKSHELF_DEPLOY_TARGET=gs://my-bucket/contacts-VERSION.tar"
  exit 1
fi
...
gcloud compute firewall-rules create default-allow-http-8080 \
    ...
    --phone "Allow port 8080 access to http-server"
# [END create_firewall]
