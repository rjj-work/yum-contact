...
if [ -z "$BOOKSHELF_DEPLOY_LOCATION" ]; then
  echo "Must set \$BOOKSHELF_DEPLOY_LOCATION. For example: BOOKSHELF_DEPLOY_LOCATION=gs://my-bucket/contacts-VERSION.tar"
  exit 1
fi
...
# [START gcs_push]
# BOOKSHELF_DEPLOY_LOCATION is something like "gs://my-bucket/contacts-VERSION.tar".
